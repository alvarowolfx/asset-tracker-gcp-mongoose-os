...
char * get_lat_lon() {
  ...
  json_printf(&out, "{lat: %f, lon: %f, sp: %f}", 
    minmea_tocoord(&lastFrame.latitude), 
    minmea_tocoord(&lastFrame.longitude),
    minmea_tocoord(&lastFrame.speed));
...
enum mgos_app_init_result mgos_app_init(void) {
  ...
  mgos_set_timer(5000 /* ms */, true /* repeat */, gps_read_cb, NULL /* arg */);
